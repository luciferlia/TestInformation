...
<body onload="data()">
			...
			<tr>
				<td><input type="hidden" name="typeId" value="${typeId }">
					...
					<div class="layui-form-item">
						...
						<div class="layui-inline">
							...
							<div class="layui-input-inline"  id="results">
								<s:if test="%{caseresult.result=='Pass'||caseresult.result=='PASS' }">
								<input type="text" name="" id="y2"value="${caseresult.result }" class="layui-input"  disabled="true" style="color:blue;" >
								
								</s:if>
								<s:else>
									<input type="text" name="" value="${caseresult.result }" class="layui-input"  disabled="true" style="color:red;" >
								</s:else>
							</div>
						</div>
						</c:if>
					...
					<div class="layui-form-item layui-form-text">
						...
						<div class="layui-input-block">
							<textarea name="cases.description" class="layui-textarea"  disabled="true" 
								style="max-width: 700px;overflow-y:visible;">${cases.remark }</textarea>
						</div>
					</div>
					<c:if test="${tag==1 }">
					<div class="layui-form-item layui-form-text"> 
	...
	<c:if test="${tag==1 }">
	<ul style="margin-left:80%;"><li style="float:left">
	<s:if test="%{startCaseId==cases.caseId }"><button class="layui-btn layui-btn-disabled layui-btn-radius">上一条</button></s:if><s:else> 
	<a href="Case_upTestCase?planId=${caseresult.plan.planId }&caseId=${cases.caseId }&tag=1">
	<button type="submit" onclick="return check()"
		class="layui-btn layui-btn-primary layui-btn-radius">上一条</button></a></s:else></li>
	<li style="float:left"><button type="submit" onclick="save()"
		class="layui-btn layui-btn-primary layui-btn-radius">确定</button></li>
	<li style="float:left">	<s:if test="%{endCaseId==cases.caseId }"><button class="layui-btn layui-btn-disabled layui-btn-radius">下一条</button></s:if><s:else> <a href="Case_nextTestCase?planId=${caseresult.plan.planId }&caseId=${cases.caseId }&tag=1" onclick="return check()"><button type="submit"onclick=""
		class="layui-btn layui-btn-primary layui-btn-radius">下一条</button></a></s:else></li></ul>
	</c:if>
	<c:if test="${tag==2 }">
	<ul style="margin-left:80%;"><li style="float:left">
	<s:if test="%{startCaseId==cases.caseId }"><button class="layui-btn layui-btn-disabled layui-btn-radius">上一条</button></s:if><s:else> 
	<a href="Case_upCase?typeId=${typeId }&caseId=${cases.caseId }&tag=2">
	<button type="submit" onclick="return check()"
		class="layui-btn layui-btn-primary layui-btn-radius">上一条</button></a></s:else></li>
	<li style="float:left">	<s:if test="%{endCaseId==cases.caseId }"><button class="layui-btn layui-btn-disabled layui-btn-radius">下一条</button></s:if><s:else> <a href="Case_nextCase?typeId=${typeId }&caseId=${cases.caseId }&tag=2" onclick="return check()"><button type="submit"onclick=""
		class="layui-btn layui-btn-primary layui-btn-radius">下一条</button></a></s:else></li></ul>
	</c:if>
	<script>
	function data(){
