...
function delet(id1){
 ...
 alert(id1);
                                layer.close(index);
                                 window.location.href='Case_deleteCaseType?id='+id1+'&tag=1';// href="Case_deleteCaseType?id=${cas.id }&tag=1"
...
<body>
	  ...
	  <table style="width:80%;margin-left:5%;margin-top:1%;"><tr><td>
			...
			<li style="float:left;"><input type="text" class="form-control" placeholder="输入客户名" id="search"
				style="width:200px;"></li>
			
			<li><c:if test="${not empty permissionForm.expor }">
			...
			<table id="table" style="TABLE-LAYOUT:fixed;WORD-BREAK:break-all;width:80%;margin-left:5%;" class="table table-condensed table-bordered table-hover table-striped">
				<tr style="background-color:#4682B4;height:40px;color: white;">
					<td style="text-align:center;vertical-align:middle;width:50px;"><h4>序号</h4></td>
					...
					<td style="text-align:center;vertical-align:middle;"><h4>创建人</h4></td>
					<td style="text-align:center;vertical-align:middle;"><h4>维护人</h4></td>
					<td style="text-align:center;vertical-align:middle;"><h4>创建时间</h4></td>
					<td style="text-align:center;vertical-align:middle;"><h4>最新维护时间</h4></td>
				...
				<c:forEach items="${caseType}" var="cas">
					<tr style="height:40px;" username="${cas.casetypeName }">
						...
						<td style="text-align:center;vertical-align:middle;"><font color="black"  size="2"><a href="Case_showCusCase?id=${cas.id }">${cas.casetypeName }</a></font></td>
						...
						<c:if test="${not empty permissionForm.edit||not empty permissionForm.delete }">
						<td style="text-align:center;vertical-align:middle;"><c:if test="${not empty permissionForm.edit }">
						<a onclick="updateRow(this);">${permissionForm.edit }</a><c:if test="${not empty permissionForm.edit&&not empty permissionForm.delete }"> | </c:if></c:if>
						<c:if test="${not empty permissionForm.delete }"><a onclick='delet(${cas.id })' >${permissionForm.delete }</a></c:if>
							</td>
							</c:if>
		...
		<h4 style="margin-left:30%"><font color="white" >修改分类</font></h4></td>
				...
				<tr>
					...
					<td><input type="text" id="l3"  name="content" class="form-control"  style="width:170px;"/></td>
				</tr>
				<tr style="display:none;">
					<td><span style="margin-left:10%;">创&nbsp;&nbsp;建&nbsp;&nbsp;人：</span></td>
					<td><input type="text" id="l4"  disabled="true" class="form-control"  style="width:170px;"/></td>
				</tr>
	...
	<div id="popDiv1"  style="display:none;" class="mydiv">
		...
		<tr style="background-color:#4682B4;">
					<td colspan="2" style="height:30px;">
						<h4 style="margin-left:40%"><font color="white" >增加分类</font></h4>
				</tr>
				<tr>
					<td><span style="margin-left:10%;">分类：</span></td>
					<td><input placeholder="必须包含客户用例四个字" type="text" name="casetype" value="" class="form-control"  style="width:200px;"></td>
				</tr>
				<tr style="background-color:#4682B4;height:30px;">
                 <td colspan="2" >
				<div style="margin-left:30%;">
